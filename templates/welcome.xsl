        ...
        <table border="1" style="width: 1000px; position: absolute; left: 50%; top: 8%; margin-left: -500px;">
            ...
            <tr>
                ...
                <td style="vertical-align: top;">
                    ...
                    <xsl:choose>
                        <xsl:when test="/root/template = 'drink'">
                            <xsl:call-template name="drink" />
                        </xsl:when>
                        <xsl:when test="/root/template = ''">
                            <xsl:call-template name="drink" />
                        </xsl:when>
                        <xsl:when test="/root/template = 'list'">
                            <xsl:call-template name="list" />
                        </xsl:when>
                        <xsl:when test="/root/template = 'ing'">
                            <xsl:call-template name="ing" />
                        </xsl:when>
                        <xsl:when test="/root/template = 'mydrinks'">
                            <xsl:call-template name="mydrinks" />
                        </xsl:when>
                        <xsl:when test="/root/template = 'bars'">
                            <xsl:call-template name="bars" />
                        </xsl:when>
                        <xsl:when test="/root/template = 'savebar'">
                            <xsl:call-template name="savebar" />
                        </xsl:when>
                        <xsl:when test="/root/template = 'editdrink'">
                            <xsl:call-template name="editdrink" />
                        </xsl:when>
                        <xsl:when test="/root/template = 'editIng'">
                            <xsl:call-template name="editIng" />
                        </xsl:when>
                        <xsl:when test="/root/template = 'editCategory'">
                            <xsl:call-template name="editCategory" />
                        </xsl:when>
                        <xsl:when test="/root/template = 'category'">
                            <xsl:call-template name="category" />
                        </xsl:when>
                        <xsl:otherwise>
                            The catchall statement is engaged. Select expected '<xsl:value-of select="/root/template" />'.
                        </xsl:otherwise>
                    </xsl:choose>
                </td>
            </tr>
